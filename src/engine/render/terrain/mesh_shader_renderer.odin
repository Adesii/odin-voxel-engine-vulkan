...
mesh_init :: proc(ctx: ^Context, r: ^vulkan.Renderer) {
	...
	bindings_desc := [?]vulkan.Descriptor_Binding {
		...
		{binding = mesh_bindings.VOXEL_MESH_BINDING_G_HEIGHTFIELDDEPTH, type = .STORAGE_IMAGE},
	...
	for index in 0 ..< vulkan.MAX_FRAMES_IN_FLIGHT {
		...
		writes := [9]vk.WriteDescriptorSet {
			...
			},
			{
				sType = .WRITE_DESCRIPTOR_SET,
				dstSet = mesh.descriptor_sets[index],
				dstBinding = mesh_bindings.VOXEL_MESH_BINDING_G_HEIGHTFIELDDEPTH,
				descriptorCount = 1,
...
mesh_update_frame_data :: proc(
	...
	total_workgroups := u64(len(mesh.bricks) * MESH_WORKGROUPS_PER_BRICK)
	assert(total_workgroups <= u64(r.mesh_shader.max_work_group_total_count))
...
mesh_record_frame :: proc(
	...
	linear_depth_barrier := vk.ImageMemoryBarrier {
		sType = .IMAGE_MEMORY_BARRIER,
		oldLayout = .GENERAL,
		newLayout = .GENERAL,
		srcQueueFamilyIndex = vk.QUEUE_FAMILY_IGNORED,
		dstQueueFamilyIndex = vk.QUEUE_FAMILY_IGNORED,
		image = ctx.linear_depth_outputs[frame_index].image,
